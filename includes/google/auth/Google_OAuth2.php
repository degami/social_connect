...
class Google_OAuth2 extends Google_Auth {
  ...
  public function __construct() {
    ...
    if (! empty($google_apiConfig['developer_key'])) {
      $this->developerKey = $google_apiConfig['developer_key'];
    }

    if (! empty($google_apiConfig['oauth2_client_id'])) {
      $this->clientId = $google_apiConfig['oauth2_client_id'];
    }

    if (! empty($google_apiConfig['oauth2_client_secret'])) {
      $this->clientSecret = $google_apiConfig['oauth2_client_secret'];
    }

    if (! empty($google_apiConfig['oauth2_redirect_uri'])) {
      $this->redirectUri = $google_apiConfig['oauth2_redirect_uri'];
    }

    if (! empty($google_apiConfig['oauth2_access_type'])) {
      $this->accessType = $google_apiConfig['oauth2_access_type'];
    }

    if (! empty($google_apiConfig['oauth2_approval_prompt'])) {
      $this->approvalPrompt = $google_apiConfig['oauth2_approval_prompt'];
    }
  }
